function [prior,finalClass, iterations,NBClassifier] = Initial_EM_spy(FeatureMatrix,PositiveSet, MixedSet)
...
initialLabels = -ones(labelCount,1);
...
while( not(isequal(oldLabels, newLabels)) ) 
    ...
    if(iterationCount>5)  % this is probably close to converging within 2,3 iterations, let alone 20, no need to continue
