...
All_features = PCApredictors(LB,:);
All_labels=[ones(size(Positive,1),1);-ones(size(Negative,1),1)];

num_folds=10; %you can change it as you wish  Use  num_folds-fold stratified cross validation
...
for i = 1:CV.NumTestSets
    ...
    %% num_fold-1 part ================================================
     % more static info
    display_moreInfo_result_Final(trainmodel_org{i,1},features,labels,['#Train fold ',num2str(i)]);
    %% AUC ROC FPR TPR
    ...
    %% remain Fold part
    % more static info
    display_moreInfo_result_Final(trainmodel_org{i,1},ts_features,ts_labels,['#Test fold ',num2str(i)]);
    %% AUC ROC FPR TPR
